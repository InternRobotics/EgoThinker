...
QUESTION_TEMPLATE_TG = """To accurately pinpoint the event "[QUESTION]" in the video, you need to identify a time interval from which the answer to the question can be deduced.\n Output your thought process within the <think> </think> tags. Then, provide the start and end times (in seconds, precise to two decimal places) in the format "(start,end)" within the <answer> </answer> tags."""


QUESTION_TEMPLATE_OG = """Locate [OBJECT] in the image. Provide your reasoning process within the <think> </think> tags. Provide your answer within the <answer> </answer> tags, following <answer>[x min, y min, x max, y max]/answer> format."""
...
class Qwen2VLGRPOTrainer_Video_QA(Trainer):
    ...
    def _get_per_token_logps_image(self, model, input_ids, attention_mask, pixel_values, image_grid_thw):
    ...
    def make_conversation_video(self, example):
        ...
        dataset_type = example['type']

        if dataset_type == 'egoexo':

            example_prompt = QUESTION_TEMPLATE_TG.replace("[QUESTION]", example["question"])
            return [
                    ...
                    {"role": "user", "content": [
                            ...
                            "total_pixels": 1792 * 28 * 28, 
                            "min_pixels": 16 * 28 * 28,
                            "fps": example["video_kwargs"]['fps'][0]
                            },
                        ]
                    },
                ]
            ...
            example_prompt = QUESTION_TEMPLATE_OG.replace("[OBJECT]",example['object'])
            gt = example['gt']
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if inputs[0]['type'] == 'visor':
            ...
            per_token_logps = self._get_per_token_logps_image(model, prompt_completion_ids, attention_mask, pixel_values, image_grid_thw)
            # Get rid of the prompt (-1 because of the shift done in get_per_token_logps)
            per_token_logps = per_token_logps[:, prompt_length - 1 :]

            with torch.inference_mode():
                if self.ref_model is not None:
                    ref_per_token_logps = self._get_per_token_logps_image(self.ref_model, prompt_completion_ids, attention_mask, pixel_values, image_grid_thw)
                else:
                    with self.accelerator.unwrap_model(model).disable_adapter():
                        ref_per_token_logps = self._get_per_token_logps_image(model, prompt_completion_ids, attention_mask, pixel_values, image_grid_thw)
        else:
